...
void generate(int open, int closed, int n, string& s, vector<string>& ans){
    ...
    //we will insert the open if the current  number of open is less than n
    if(open < n) {
        s.push_back('(');
    ...
    //we will insert the closing bracket only if open > closed
    if(closed < open) {
        s.push_back(')');
...
void solve(){
    ...
    vector<string> ans;
    string s = "";
    //let the number of open == closed and length of string is n
